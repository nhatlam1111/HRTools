...
#ACCESS
[ACCESS_SELECT_ATTENDANCE]=SELECT CAST(USERID AS VARCHAR) AS USER_ID,'' AS CARD_ID,CONVERT(VARCHAR(8), CHECKTIME, 112) AS WORK_DATE,CONVERT(VARCHAR(5), CHECKTIME, 108) AS WORK_TIME,FORMAT(CHECKTIME, 'yyyyMMddHHmmss') AS WORK_DATE_FULL,CHECKTYPE AS EVT,sn AS MACHINE_ID FROM CHECKINOUT WHERE CONVERT(VARCHAR(8), CHECKTIME, 112) BETWEEN '$[from_dt]' and '$[to_dt]'
[ACCESS_SELECT_USER]
[ACCESS_INSERT_USER]
