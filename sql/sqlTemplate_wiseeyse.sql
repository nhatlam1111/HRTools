...
#ACCESS
[ACCESS_SELECT_ATTENDANCE]=select CStr(UserEnrollNumber) as USER_ID, '' AS CARD_ID, Format(TimeStr, 'yyyymmdd') as WORK_DATE, Format(TimeStr, 'HH:mm') as WORK_TIME, Format(TimeStr, 'yyyymmddHHmmss') as WORK_DATE_FULL, OriginType as EVT, MachineNo as MACHINE_ID from CHECKINOUT where Format(TimeStr, 'yyyymmdd') between '$[from_dt]' and '$[to_dt]'
[ACCESS_SELECT_USER]
[ACCESS_INSERT_USER]
